...
def episode_wise_data(df: pd.DataFrame) -> Tuple[List[np.ndarray], List[np.ndarray]]:
...
def epi_pha_data(df: pd.DataFrame) -> Tuple[List[np.ndarray], List[np.ndarray]]:
    episodes: List[np.ndarray] = []
    targets: List[np.ndarray] = []
...
def build_dataloaders(
    ...
    # 여기서는 노트북과 동일하게 phase_wise_data 를 사용
...
def run_inference(
    ...
    for _, row in tqdm(submission.iterrows(), total=len(submission)):
        ...
        # -----------------------------
        # 노트북에서 case 1 만 사용하던 로직과 동일
        # (마지막 phase 길이가 1인 경우에 대한 예외 처리는 주석 처리된 상태)
        # -----------------------------
...
def main():
    ...
    # 추론 및 제출 파일 생성
    run_inference(model, best_state)
