...
def run_training():
    ...
    # [중요] 범주형 변수의 이름(혹은 인덱스) 리스트
    # Player ID, Team ID 등을 CatBoost가 알아서 인코딩하게 맡깁니다.
    cat_features = ['type_name', 'prev1_type', 'team_id', 'prev1_team', 'player_id']
    
    # 데이터셋에 없는 컬럼은 제외
    cat_features = [c for c in cat_features if c in X_train.columns]
    
    # CatBoost 데이터셋 객체 생성 (Pool)
    train_pool_x = Pool(X_train, y_x_train, cat_features=cat_features)
    val_pool_x = Pool(X_val, y_x_val, cat_features=cat_features)
    
    train_pool_y = Pool(X_train, y_y_train, cat_features=cat_features)
    val_pool_y = Pool(X_val, y_y_val, cat_features=cat_features)
    ...
    cb_params = {
        ...
        'task_type': 'GPU', # GPU 사용 가능하면 'GPU' (없으면 'CPU')
        'devices': '0',
        'verbose': 200
    }
    
    print("\n🚀 Training CatBoost X Model...")
    model_x = CatBoostRegressor(**cb_params)
    model_x.fit(train_pool_x, eval_set=val_pool_x)
    
    print("\n🚀 Training CatBoost Y Model...")
    model_y = CatBoostRegressor(**cb_params)
    model_y.fit(train_pool_y, eval_set=val_pool_y)
    ...
    print(f"\n   >>> Validation Mean Distance Error: {avg_dist:.4f} m")
    
    model_x.save_model(os.path.join(Config.MODEL_DIR, 'catboost_x.cbm'))
    model_y.save_model(os.path.join(Config.MODEL_DIR, 'catboost_y.cbm'))
