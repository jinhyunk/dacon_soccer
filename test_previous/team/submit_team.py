...
class Config:
    ...
    # [경로 설정]
    TRAIN_DIR = './data_test/train'   # 팀 매핑 생성을 위해 필요
    TEST_DIR = './open_track1/test'   # 테스트 데이터
    WEIGHT_DIR = './weights'
    MODEL_NAME = 'team_best.pth'      # [중요] 학습한 팀 모델 파일명
    OUTPUT_FILE = './submission_team.csv'
    ...
    def __getitem__(self, idx):
        try:
            ...
            if len(phase_end_coords) > 0:
                ...
                # Dummy Team ID if no history
                phase_teams.append(0) 

            return (phases_data, start_actions, phase_lens, torch.FloatTensor(phase_end_coords), 
                    torch.LongTensor(phase_teams), game_episode_id)
...
def test_collate_fn(batch):
    ...
    b_phases, b_acts, b_lens, b_coords, b_teams, b_ids = zip(*batch)
    ...
    return pad_phases, phase_lengths, episode_lengths, start_action_ids, phase_len_ids, padded_coords, phase_team_ids, b_ids

# ==========================================
# 3. Inference Engine
# ==========================================
def build_team_mapping(data_dir):
    """ 학습 때와 동일한 Team ID 매핑 생성 (Train 폴더 스캔) """
    print("🔄 Building Team ID Mapping (from Train Data)...")
    files = glob.glob(os.path.join(data_dir, '*.csv'))
    unique_teams = set()
    
    # 샘플링 스캔 (속도 위해 2000개만)
    # [주의] 만약 학습 시 전체를 다 읽었다면 여기도 전체를 읽어야 ID 순서가 보장됨.
    # 하지만 보통 팀 ID는 정수이므로 sorted()하면 순서는 동일하게 유지됨.
...
def run_inference():
    ...
    print(f"🔄 Loading Team model from {model_path}...")
    try:
    ...
    with torch.no_grad():
        for batch in tqdm(test_loader, desc="Predicting"):
            ...
            phase_team_ids = batch[6].to(Config.DEVICE)
            ids = batch[7]
            
            # Forward: (pred, last_known) 반환
            preds, _ = model(padded_phases, phase_lengths, episode_lengths, 
                             start_action_ids, phase_len_ids, padded_coords, phase_team_ids)
