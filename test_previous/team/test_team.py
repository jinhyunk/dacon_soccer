...
class LocationAwareHierarchicalLSTM(nn.Module):
    ...
    def forward(self, padded_phases, phase_lengths, episode_lengths, start_action_ids, phase_len_ids, padded_coords, phase_team_ids):
        ...
        # Context Vector 결합 (Action + Length + Team)
        ...
        # 4. Regression
        predicted_remaining_delta = self.regressor(episode_context)
...
# ==========================================
class TeamAwareDataset(Dataset):
    ...
    def __getitem__(self, idx):
        try:
            ...
            return (phases_data, torch.FloatTensor(target), start_actions, phase_lens, 
                    torch.FloatTensor(phase_end_coords), torch.LongTensor(phase_teams))
        except: return None

def team_collate_fn(batch):
    batch = [x for x in batch if x is not None]
    if not batch: return (None,)*7 # 인자 개수 7개로 맞춤
    
    b_phases, b_targets, b_acts, b_lens, b_coords, b_teams = zip(*batch)
    ...
    return pad_phases, phase_lengths, episode_lengths, targets, start_action_ids, phase_len_ids, padded_coords, phase_team_ids
...
def run_training():
    ...
    print("📂 Team Embedding Only 학습 시작...")
    
    train_dataset = TeamAwareDataset(Config.TRAIN_DIR)
    val_dataset = TeamAwareDataset(Config.VAL_DIR)
    
    train_loader = DataLoader(train_dataset, batch_size=Config.BATCH_SIZE, shuffle=True, 
                              collate_fn=team_collate_fn, num_workers=Config.NUM_WORKERS)
