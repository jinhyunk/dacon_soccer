...
class Config:
    ...
    # 모델 파라미터 (test_guassian.py와 동일해야 함)
    NUM_ACTIONS = 33
    ...
    PHASE_HIDDEN = 64
    EPISODE_HIDDEN = 256    
    DROPOUT = 0.0           # Inference 시에는 0.0
    
    # [경로 설정]
    TEST_DIR = './open_track1/test'   # 테스트 데이터 폴더
    WEIGHT_DIR = './weights'
    MODEL_NAME = 'Gaussian_last.pth'  # [중요] 학습한 가우시안 모델 파일명 확인!
    OUTPUT_FILE = './submission_gaussian.csv'
...
# ==========================================
# 1. Uncertainty Model Definition
# ==========================================
# test_guassian.py 에 있는 모델 구조를 그대로 가져옴
class UncertaintyLocationAwareLSTM(nn.Module):
    def __init__(self, input_size=5, phase_hidden=64, episode_hidden=256, output_size=2, dropout=0.3,
        ...
        # Regressor Head (Output: 4 -> mu_x, mu_y, log_var_x, log_var_y)
        self.regressor = nn.Sequential(
            nn.Linear(episode_hidden, episode_hidden // 2),
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Linear(episode_hidden // 2, output_size * 2) 
        )
...
def run_inference():
    ...
    print("🚀 Starting Gaussian Inference...")
    with torch.no_grad():
        for batch in tqdm(test_loader, desc="Predicting"):
            ...
            # Forward: (mu, sigma) 반환
            preds_mu, preds_sigma = model(padded_phases, phase_lengths, episode_lengths, 
                                          start_action_ids, phase_len_ids, padded_coords)
            
            # Submission에는 예측된 좌표(Mu)만 사용
            # Denormalize
            pred_x = preds_mu[:, 0].cpu().numpy() * Config.MAX_X
            pred_y = preds_mu[:, 1].cpu().numpy() * Config.MAX_Y
